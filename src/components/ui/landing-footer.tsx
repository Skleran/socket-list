...
export default function LandingFooter() {
  ...
  return (
    <div className="w-full pt-6 flex flex-col items-center gap-15 sm:gap-12 justify-between border-t-1">
      <div className="flex flex-col items-center">
        <div className="text-center text-5xl font-bold tracking-tight leading-12.5">
          Skleran List
        </div>
        <div className="text-center">{t("Footer.desc")}</div>
        <div className="flex flex-col max-w-[340px] sm:flex-row sm:max-w-fit sm:items-center gap-2 mt-8 px-10">
          <Button
            className="py-5 sm:w-[240px]"
            onClick={() => {
          ...
          >
            <div className="grid grid-cols-[1fr_1fr_1fr] gap-3">
              ...
              <div
                className={`flex gap-1 items-center ${locale === "tr" ? "flex-row-reverse" : ""}`}
              >
          ...
          <p className="hidden sm:inline">{t("Footer.or")}</p>
          <Button
            className="py-5 sm:w-[240px]"
            onClick={() => {
      ...
      <div
        className={`flex gap-1 ${locale === "tr" ? "flex-row-reverse" : "flex-row"}`}
      >
        {t("Footer.dev_with")}{" "}
        <Heart className="fill-destructive text-destructive" /> {t("Footer.by")}{" "}
        <span className="underline">
          <a href="https://github.com/Skleran" target="_blank">
            skleran
          </a>
        </span>
      </div>
